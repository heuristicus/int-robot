	...
	public class LaserPlotPanel extends JPanel {
    	...
    	public void paint(Graphics g) {
    		...
    		for (int i = 0; i < latestReadings.length; i++) {
                reading = latestReadings[i];
    			if (reading != 0) {
    				g.drawOval(i, (int)(reading * 100), PLOT_POINT_WIDTH, PLOT_POINT_HEIGHT);
                } else { // We draw zeros differently
                	g.setColor(ZERO_POINT_COLOR);
                	g.drawOval(i, (int)(reading * 100), ZERO_POINT_WIDTH, ZERO_POINT_HEIGHT);
                	g.setColor(PLOT_POINT_COLOR);
                }
            }
    	}
