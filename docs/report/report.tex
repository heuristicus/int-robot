...
navigation to person using kinect depth data
...
\begin{figure}
  ...
  \caption{ROC analysis of frontal\_face haar cascades.}
  \label{fig:haar}
\end{figure}
\subsubsection{Aim}
...
\subsubsection{Method}
To test the different Haar cascades we captured a set of images using the Kinect camera which all included faces, some of which were in different surroundings and some of which include a face at different angles. Once all images had been generated, we passed them into our face detection node which uses the cascades to perform face detection on still images. Firstly, we changed the cascade from within the code to use one of the following: frontalface\_default, frontalface\_alt, frontalface\_alt2 and frontalface\_alt\_tree. We then passed in the images we had generated and saved the output of the face detection. The face detection node drew rectangles around areas it believed were faces which allowed us to perform manual analysis and present the data in an understandable manner. In order to generate the data, we looked at the image output from the program for each cascade and analysed the number of faces that were correctly detected (true positives), the number of non faces not detected (true negatives), the number of falsely detected faces (false positives) and finally, the number of faces which were completely missed (false negatives). Once we had all these values for each image, we averaged the sensitivity for each Haar cascade to find their final value.

\subsubsection{Results}
Form our results of the above experimentation it was obvious all cascades did not perform well on profile faces (I.e faces at an near 90 degree angle). This was expected as the cascades we used were specifically trained on frontal face images and therefore the detection was failing. As soon as enough information could be gathered from the image regarding the face, the default and alt cascades both detected the face and performed consistently. However, the other two detectors required the face to be at less of an angle in order to be detected. On the images which had different surroundings we found that again the default cascade performed rather well as did the alt 2 cascade. Overall the best performing cascade from averaged sensitivity was the default cascade which had a value of 0.758. Sensitivity is the ability of the cascade to detect and classify faces correctly. From the results this cascade did consistently find most or all images, however it would also consistently wrongly classify non face objects as faces, which can be seen from the 1-specificity value which was 0.76. What this value shows is that the number of false positives was rather high, i.e. it was detecting faces when there were no faces to detect. 

\subsubsection{Conclusion}
In conclusion, from the data gathered from the experiment we can see that the Haar cascade which performed above other cascades in detecting faces was the frontalface\_default. We saw that this cascade tended to classify non face objects as faces (false positives), however it did, with a good degree of consistency, classify actual faces as faces (true positives). As we decided to use the Kinect as a camera, we were aware it is possible to extract depth information for the object in view. Given the large number of false positives produced by the face detection with all cascades, we decided to use the depth information to identify false positives, by using knowledge of the size of human faces. To do so, we developed a model which helps prune non-face objects which are picked up by the cascade. We believe it to be more sensible to gather more correctly detected faces alongside non-face objects (false positives) which we can prune out, as opposed to a lower number of correctly detected faces (true negatives) and a very small number of detected faces (false positives). The frontalface\_alt which also performed well was not chosen because its performance in sensitivity, a value of 0.744 was lower than that of frontalface\_default which shows that it was not as good as detecting faces as the frontalface\_default cascade was. However the 1-specificity value of frontalface\_alt, 0.122, was the best performing amongst all cascades and showed this cascades ability to oly detect true faces. In summary we chose to use the frontalface\_default cascade as it performed well at detecting true faces, and even though its false positive rate was rather high, we  built a model to prune non face objects. 
...
\begin{figure}
  ...
  \caption{ROC analysis of frontal\_face\_alt cascade with assumption and depth models applied.}
  \label{fig:haar}
\end{figure}
\begin{figure}
  \includegraphics[width=\columnwidth]{centre_model}
  \caption{Depth model for the centre of the field of view.}
  \label{fig:haar}
\end{figure}
